...
class (Enum a, Bounded a, Eq a) => CyclicEnum a where
  csucc :: a -> a
  csucc x
    | maxBound == x = minBound
    | otherwise = succ x
...
data QuoteState = Quoted | Unquoted deriving (Eq, Enum, Bounded)

instance CyclicEnum QuoteState
...
doToLine quotestate word ('"' : xs) = doToLine (csucc quotestate) ('"' : word) xs
